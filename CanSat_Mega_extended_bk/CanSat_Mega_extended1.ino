...
void setup() {
 ...
 //  while (!RF_port.available());                 // wait for data
  // while (RF_port.available()); // empty buffer again
   ...
   RF_port.println(F("Initializing DMP..."));
   ...
   // while (RF_port.available() && RF_port.read()); // empty buffer
   // while (!RF_port.available());                 // wait for data
   // while (RF_port.available() && RF_port.read()); // empty buffer again
    ...
    pinMode(LED_PIN, OUTPUT);
	    ...
	    // verify connection
	    RF_port.println("Testing device connections...");
	    RF_port.println(mag.testConnection() ? "HMC5883L connection successful" : "HMC5883L connection failed");
